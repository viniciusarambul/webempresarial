...
var audio = {
     ...
     * @returns void
     */
    remover: function (idUnidade, idAudio) {
        app.deletarDialog("Deseja realmente deletar este áudio?", function () {
            var url = '/unidades/' + idUnidade + '/audios/' + idAudio;
            ...
            app.request(
                    ...
                    function () {
                        app.alert('Falha ao remover áudio, tente novamente.');
                    }
            );
        });
    },
     ...
     * @returns void
     */
    editar: function (idUnidade, idAudio) {
        var url = '/unidades/' + idUnidade + '/audios/' + idAudio;
        ...
        app.request(
                ...
                function (res) {
                    ...
                    if (res) {
                        ...
                        $('#idAudio').val(res.idAudio);
                        $('#nomeAudio').val(res.nome);
                        tinyMCE.get('descricaoAudio').setContent(res.descricao);
                        $('#urlAudio').val(res.url);

                        $('#modal-audios').modal('open');
                    } else {
                        app.alert('Ops, audio não encontrado.');
                    }
                },
                function () {
                    app.alert('Falha ao carregar dados do áudio, tente novamente.');
                }
        );
    },
    ...
    inserir: function () {
        ...
        $('#idAudio').val('');
        $('#nomeAudio').val('');
        tinyMCE.get('descricaoAudio').setContent('');
        $('#urlAudio').val('');
